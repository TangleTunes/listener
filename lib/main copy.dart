...
void main() async {
  ...
  Credentials ownCredentials = EthPrivateKey.fromHex(privateKey);
  //-------------------------------------------------------------------
  PrivateKeyNotifier pkNotifier = PrivateKeyNotifier();
  runApp(
    ChangeNotifierProvider(
      create: (context) => pkNotifier,
      child: const MyApp(),
    ),
  );
  await Future.delayed(Duration(seconds: 2)); //sleep

  pkNotifier.setOwnCredentials(ownCredentials);
}

class PrivateKeyNotifier extends ChangeNotifier {
  late Credentials thisCredentials;
  void setOwnCredentials(Credentials credentials) {
    thisCredentials = credentials;
    notifyListeners();
  }
}
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Widget initialRoute = MyHomePage();

    return MaterialApp(
      initialRoute: '/',
      routes: {'/': (context) => initialRoute},
    );
  }
}
            ...
            children: [
              ...
              Consumer<Credentials>(
                builder: (context, cart, child) {
                  return Text("credetilasa are ${ownCredentials.address}");
                },
              ),
